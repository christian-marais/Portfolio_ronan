...
class Router{
    ...
    private static function setRedirection(){
        ...
        if($params[0]!=""){
            ...
            if(!file_exists($controller_path.$controller.'.php')){
                require_once(ROOT."src\\views\pages\\404.php");
            }
        }else{
            $controller="Pages";
            $action="index";
        ...
        (empty(new $controller))?self::redirection('pages/erreur404'):"";
        $controller = new $controller();
        if(method_exists($controller,$action)){
            unset ($params[0],$params[1]);
            call_user_func_array([$controller,$action],$params);
        }else{
           self::redirection('pages/erreur404');
        }
    }   
    ...
    public static function redirection($redirection){
        header('Location: '.BASE_URI.$redirection.Security::$uriToken);
    } 
}
?>