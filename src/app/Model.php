...
abstract class Model extends DB{
    ...
    protected function getConnection(){
        if(self::$pdo===null){
            ...
            try{
                parent::setEnv();
                self::$pdo = new PDO('mysql:local='.$this->host.'; dbname=c1865308c_aati', "c1865308c_christian","@87Toopac87",array(PDO::ATTR_PERSISTENT    => true));
    ...
    protected function bindParamSearchAndOffset(&$query,&$addSql,&$valeurs,$p=1){
        if($addSql!=null && !empty($valeurs['search'])){
            $p=0;
            foreach($valeurs['search'] as $key){
                $query->bindParam(":search$p",$key,PDO::PARAM_STR);
                $p++;
            }
        }
    ...
    protected function setSearch($champ1,$champ2,&$search,&$sql,&$data,$option='AND'){
        if($search!=null && is_array($champ1) && $champ2===null){
            $p=0;
            foreach($champ1 as $key){
                if($p===0){
                    $sql=" $option $key LIKE CONCAT('%',:search0,'%')";
                    $data['search']=array($search);
                }else{
                    $sql.=" OR $key LIKE CONCAT('%',:search$p,'%')";
